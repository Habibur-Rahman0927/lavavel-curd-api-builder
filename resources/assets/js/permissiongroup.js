    ...
    var table = $('.yajra-datatable').DataTable({
        ...
        columns: [
            ...
            render: function(data, type, row) {
                let editButton = '<a href="/admin/permissiongroup/' + row.id + '/edit" class="edit btn btn-success btn-sm">Edit</a>';
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status_code === 200) {
                            Swal.fire(
                                'Deleted!',
                                'The permissiongroup has been deleted.',
                                'success'
                            );
                            table.row(row).remove().draw();
                             ...
                             Swal.fire(
                                'Error!',
                                response.message || 'PermissionGroup was not deleted.',
                                'error'
                            );
                        }
                    },
                    error: function (xhr) {
                        Swal.fire(
                            'Error!',
                            'There was an error deleting the permissiongroup.',
                            'error'
                        );
                    }
                });
