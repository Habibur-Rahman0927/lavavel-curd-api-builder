    ...
    var table = $('.yajra-datatable').DataTable({
        ...
        columns: [
            ...
            render: function(data, type, row) {
                let editButton = '<a href="/admin/{{ name }}/' + row.id + '/edit" class="edit btn btn-success btn-sm">Edit</a>';
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status_code === 200) {
                            ...
                            Swal.fire(
                                'Error!',
                                response.message || '{{ name }} was not deleted.',
                                'error'
                            );
                        }
                    },
                    error: function (xhr) {
                        Swal.fire(
                            'Error!',
                            'There was an error deleting the {{ name }}.',
                            'error'
                        );
                    }
                });
